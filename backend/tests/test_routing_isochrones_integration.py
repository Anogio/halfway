...
class RoutingIsochronesIntegrationTest(unittest.TestCase):
    def test_compute_isochrones_returns_scalar_grid(self) -> None:
        runtime = RuntimeData(
            version="vtest",
        ...
        self.assertIn("scalar_grid", result)
        self.assertNotIn("feature_collection", result)
        self.assertEqual(result["scalar_grid"]["grid"]["row_count"], 2)
        self.assertEqual(result["scalar_grid"]["grid"]["col_count"], 2)
    ...
    def test_compute_isochrones_decouples_compute_and_render_horizons(self) -> None:
        ...
        scalar_grid = result["scalar_grid"]
        self.assertIsNotNone(scalar_grid)
        assert scalar_grid is not None
        for value in scalar_grid["grid"]["values"]:
            if value is not None:
                self.assertLessEqual(value, 600)
    ...
    def test_compute_multi_isochrones_reconciles_with_max_and_intersection(self) -> None:
        ...
        self.assertIsNotNone(result["scalar_grid"])
    ...
    def test_compute_multi_isochrones_empty_intersection_returns_empty_features(self) -> None:
        ...
        self.assertIsNone(result["scalar_grid"])

    def test_compute_multi_isochrones_returns_scalar_grid(self) -> None:
        runtime = RuntimeData(
            version="vtest",
            profile="weekday_non_holiday",
            nodes=[
        ...
        result = compute_multi_isochrones(
            ...
            spatial,
            topology=topology,
            origins=[{"id": "origin-1", "lat": 48.8000, "lon": 2.3000}],
            first_mile_radius_m=120,
            first_mile_fallback_k=1,
            max_seed_nodes=24,
            walk_speed_mps=1.2,
